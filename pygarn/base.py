...
import numpy as np
import copy
from typing import TypeVar, Type, Set, Union
...
def get_unused_vertex(graph: T_Graph, inplace=True):
    ...
    if graph.has_node(vertex_new):
        graph = nx.relabel_nodes(graph, {name: ix for ix, name in enumerate(graph.nodes)}, copy=not inplace)
...
def sampler_random_uniform(candidates: Union[int,str], min: int, max: int):
    if min > len(candidates):
        raise ValueError(f"Can not sample from a candidate list of size {len(candidates)} which is smaller than the given minimum {min}")
    if max is None:
        max = len(candidates)
    elif max < min:
        raise ValueError(f"Minimum {min} is larger than given maximum {max} for range [{min},{max}].")
...
class VertexSelector(object):
    ...
    def forward_sample(self, graph: T_Graph) -> Union[int,str]:
        return self._sampler(self.forward_suggest(graph), min=self._sample_min, max=self._sample_max)
...
class VertexDegreeSelector(VertexSelector):
    def __init__(self, descending: bool=True, limit: int = 3, min: int = 1, min_degree: int=None, max_degree: int=None, sampler=np.random.choice):
    ...
    def forward_suggest(self, graph: T_Graph) -> Set[Union[int,str]]:
        degree_vertices = [(v, d) for v, d in graph.degree() if (self._min_degree is not None and d >= self._min_degree) and (self._max_degree is not None and d <= self._max_degree)]
        order_vertices = [v for (v, d) in sorted(degree_vertices, key=lambda tup: tup[1], reverse=self._descending)]
