...
def grow_dense_high_degree_edges(graph: nx.Graph, k: int = 0, n_sel: int = None):
    ...
    for i in range(k):
        ...
        degrees = np.array(
            sorted(
                [(v, d) for (v, d) in graph.degree() if d < len(graph.nodes) - 1],
                key=lambda x: x[1],
                reverse=True,
            )
        )
...
def shrink_sparsify_highest_degrees(graph: nx.Graph, k: int = 0):
    ...
    assert k > 0
    degrees = sorted(
        [(v, d) for (v, d) in graph.degree()], key=lambda x: x[1], reverse=True
    )
...
def operation_modify_k_edges(graph: nx.Graph, mode="dense", k: int = 1):
    ...
    edge_value = (
        0 if mode == "dense" else 1
    )  # use "dense" to add edges, other values to remove edges
    edge_indices = np.where(A == edge_value)
    if len(edge_indices[0]) < 1:  # There are no edges we can add/remove
        return
    if len(edge_indices[0]) < k:
        k = len(edge_indices)
    edge_choices = np.random.choice(
        len(edge_indices[0]), k, replace=False
    )  # np.random.randint(0, len(unconnected_indices[0]))
